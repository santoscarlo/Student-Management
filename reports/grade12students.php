...
// HEADER TITLE
    ...
    $this->CELL(0,0,'GRADE 12 STUDENTS LIST',0,0,'C');
    $this->Ln();
...
function viewTable($db){
    $this->SetFont('Times','',12);
    $stmt = $db->query('SELECT * from studenttable WHERE yearlevel = "GRADE 12 SHS" order by lastname ASC ');
...
// TOTAL RECORD COUNT
          ...
          $sql="SELECT count(*) from studenttable WHERE yearlevel = 'GRADE 12 SHS'";
          $result=mysqli_query($conn,$sql);
          $row=mysqli_fetch_array($result);
          // echo "$row[0]";
              $this->SetFont('Times','B',12);
             $this->Cell('B',10, 'Total Grade 12 Students: '.$row[0] ,True, 'L',0);
          mysqli_close($conn);
