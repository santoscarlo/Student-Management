...
// HEADER TITLE
    ...
    $this->CELL(0,0,'GRADE 11 STUDENTS LIST',0,0,'C');
    $this->Ln();
...
function headerTable(){
 ...
 $this->Cell(28,10,'Strand',1,0,'C');
 $this->Cell(49,10,'Year Level',1,0,'C');
 $this->Ln();

}
function viewTable($db){
    $this->SetFont('Times','',12);
    $stmt = $db->query('SELECT * from studenttable WHERE yearlevel = "GRADE 11 SHS" order by lastname ASC ');
    ...
    $this->Cell(49,10,$data->yearlevel,1,0,'L');
...
// TOTAL RECORD COUNT
          ...
          $sql="SELECT count(*) from studenttable WHERE yearlevel = 'GRADE 11 SHS'";
          $result=mysqli_query($conn,$sql);
          $row=mysqli_fetch_array($result);
          // echo "$row[0]";
              $this->SetFont('Times','B',12);
             $this->Cell('B',12, 'Total Grade 11 Students: '.$row[0] ,True, 'L',0);
          mysqli_close($conn);
}
