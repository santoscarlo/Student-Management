...
// HEADER TITLE
    ...
    $this->CELL(0,0,'NOT ENROLLED STUDENT LIST',0,0,'C');
    $this->Ln();
...
function viewTable($db){
    $this->SetFont('Times','',12);
    $stmt = $db->query('SELECT * from studenttable WHERE statusofstud IN ("Not Enrolled")order by lastname ASC ');
...
// TOTAL RECORD COUNT
          ...
          $sql="SELECT count(*) from studenttable WHERE statusofstud IN ('Not Enrolled')";
          $result=mysqli_query($conn,$sql);
          $row=mysqli_fetch_array($result);
          // echo "$row[0]";
              $this->SetFont('Times','B',12);
             $this->Cell('B',12, 'Total of Not Enrolled: '.$row[0] ,True, 'L',0);
          mysqli_close($conn);
}
